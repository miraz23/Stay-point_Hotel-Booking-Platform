  ...
  return (
      <div className="text-center my-30 mx-4 md:mx-10">
          {loading ? (
            ...
            <div>
              <div className='w-full flex flex-col md:flex-row gap-4 md:gap-x-10'>
                ...
                <div>
                  ...
                  {hotel.rooms && hotel.rooms.length > 0 ? (
                    <div className='flex gap-4 md:gap-5 mt-5 flex-wrap'>
                      ...
                      <div className="w-full md:w-[calc(50%-12px)] flex flex-col justify-between border-1 border-gray-200 rounded-lg p-3 md:p-4 bg-white shadow-2xl" key={room.id}>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 mb-4 md:mb-5">
                            ...
                            <span className="flex text-gray-600 text-xs md:text-sm font-semibold items-center"><IconCalendarCheck size={18} className='mr-1'/> {room.total_rooms} Available</span>
                        </div>
                        <div>
                        ...
                        <div className='flex flex-col md:flex-row gap-4 pb-4 md:pb-5 text-left text-gray-600'>
                          <div className='flex-1'>
                            <label className="block text-gray-700 text-sm font-semibold mb-2">Check-in</label>
                            <input 
                              type="date" 
                              value={roomDates[room.id]?.checkIn || ''} 
                          ...
                          <div className='flex-1'>
                            ...
                            <input 
                              type="date" 
                              value={roomDates[room.id]?.checkOut || ''} 
                              onChange={(e) => handleDateChange(room.id, 'checkOut', e.target.value)}  
                              min={roomDates[room.id]?.checkIn || new Date().toISOString().split('T')[0]}  
                              className="w-full px-3 md:px-4 py-2 border border-gray-300 rounded-lg text-sm md:text-base" 
                            />
                          </div>
                        </div>
                        <div className='w-full'>
                          <button 
                            className={`w-full px-4 md:px-6 py-2 text-white text-sm md:text-base font-semibold rounded-lg shadow-md ${!roomDates[room.id]?.checkIn || !roomDates[room.id]?.checkOut || room.total_rooms <= 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-cyan-500 hover:opacity-90 transition cursor-pointer'} flex items-center justify-center gap-2`}
                    ...
                    <div className='flex flex-col justify-center items-center p-6 md:p-10'>
                      ...
                      <p className='text-lg md:text-xl text-gray-400'>No rooms available. Check again later.</p>
                    </div>
                  )}
                </div>
              </div>
